...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    // Slot requires exactly one child element, so the spinner (and even a
    // literal `false`) must never be rendered alongside it.
    ...
    return (
      <Comp
        ...
        ref={ref}
        // `disabled` is invalid on non-button elements rendered via Slot
        {...(useSlot ? { disabled } : { disabled: disabled || loading })}
        {...props}
      >
        {useSlot ? (
          children
